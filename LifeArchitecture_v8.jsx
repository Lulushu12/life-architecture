...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      try{const lr=await window.storage.get("la_long_v8");if(lr?.value)setLongQ(JSON.parse(lr.value));
      const dr=await window.storage.get("la_daily_v8");if(dr?.value)setDailyQ(JSON.parse(dr.value));}catch(_){}
    })();
  },[]);

  const saveLong=async(q)=>{setLongQ(q);try{await window.storage.set("la_long_v8",JSON.stringify(q));}catch(_){}};
  const saveDaily=async(q)=>{setDailyQ(q);try{await window.storage.set("la_daily_v8",JSON.stringify(q));}catch(_){}};
