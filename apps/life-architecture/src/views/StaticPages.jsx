        ...
        <table className="tbl">
          ...
          <tbody>
            {[["1st", "Cue", "Obvious", "Invisible"], ["2nd", "Craving", "Attractive", "Unattractive"], ["3rd", "Response", "Easy", "Difficult"], ["4th", "Reward", "Satisfying", "Unsatisfying"]].map(([l, c, b, r]) => (
              <tr key={l}><td><span className="law-n">{l} Law</span></td><td style={{ color: "var(--tx2)" }}>{c}</td><td><span className="bld">{b}</span></td><td><span className="brk">{r}</span></td></tr>
            ))}
          </tbody>
        </table>
      </div>
...
function questFor(o, longQ) {
  const list = Array.isArray(longQ) ? longQ.filter(q => q && typeof q === "object") : [];
  const byId = list.find(q => q.id === o.qid);
  if (byId && o.match.test(String(byId.title || ""))) return byId;
  return list.find(q => o.match.test(String(q.title || ""))) || byId || null;
}

export function POutputs({ longQ = [] }) {
  const completed = OUTPUTS.filter(o => questFor(o, longQ)?.status === "Completed").length;
  return (
    <>
      ...
      {OUTPUTS.map(o => {
        const isDone = questFor(o, longQ)?.status === "Completed";
        return (
